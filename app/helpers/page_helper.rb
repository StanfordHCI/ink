module PageHelper
  ...
  def link_to_add_panels(pic, f, association)
    ...
    fields = f.fields_for(association, new_object, child_index: id) do |builder|
      render(association.to_s.singularize + "_fields", f: builder, panel: new_object)
    end
  ...
  def add_tags(f, association, option_name, page_id, panel_type)
    new_object = f.object.send(association).klass.new
    id = new_object.object_id
    new_object.name = option_name
    new_object.page_id = page_id
    new_object.panel_type = panel_type 
